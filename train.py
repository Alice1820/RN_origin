...
tf.app.flags.DEFINE_string('train_dir', 'train_rev', """Directory where to write event logs and checkpoint.""")
...
#        logits, cnn_features = model.inference(images, sentences, answers)
        logits, predict_labels = model.inference_demo(images, sentences, answers)
        # Calculate loss
...
#        acc = model.accuracy(logits, answers)
        ...
        sess = tf.Session()
        sess.run(init)
        saver.restore(sess, "train_rev/model.ckpt-32000")
...
#            predict_labels, accuracy_value = model.accuracy(logits_value, answers, embedding_matrix_value)
